...
- `.codex/README.md` 会被跟踪；`.codex/skills` 和 `.codex/skills-config.json` 是本地生成文件。
...
仓库只会跟踪 `.codex/README.md`。请在 clone 后根据当前平台，在本地生成 `.codex/skills` 与 `.codex/skills-config.json`：
...
- `.claude/skills` 是唯一维护源。
- `.codex/skills` 和 `.codex/skills-config.json` 是本地生成文件；Codex 用户在 clone 后应运行 `scripts/setup_shared_skills.py`。
